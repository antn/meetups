...
// Usage in a view:
//   <day-tabs>
//     <button data-targets="day-tabs.tabs" data-action="click:day-tabs#select" data-index="0">…</button>
//     …
...
// chosen day is also written to the URL (?day=N) and onto any [data-day-link]
// (the tag-filter chips) so that a filter reload keeps the viewer on the same day.
class DayTabsElement extends HTMLElement {
  connectedCallback() {
    const current = this.tabs.findIndex((tab) => tab.getAttribute("aria-selected") === "true")
    this.syncLinks(current < 0 ? 0 : current)
  }
  ...
  select(event) {
    ...
    url.searchParams.set("day", index)
    window.history.replaceState({}, "", url)

    this.syncLinks(index)
  }
